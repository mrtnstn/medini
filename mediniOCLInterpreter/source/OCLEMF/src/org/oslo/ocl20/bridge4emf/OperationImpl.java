...
public class OperationImpl extends
		org.oslo.ocl20.semantics.bridge.impl.OperationImpl implements Operation {
	...
	public Classifier getReturnType() {
		...
		// return
		// this.processor.getBridgeFactory().buildClassifier(_impl.getEType());
		Classifier _type;
		EClassifier etype = this._impl.getEType();
		Classifier type = this.processor.getBridgeFactory().buildClassifier(
				etype);
		if (this._impl.isMany()) {
			if (this._impl.isUnique()) {
				_type = this.processor.getTypeFactory().buildOrderedSetType(
						type);
			} else {
		...
		if (this._ownedParameters == null) {
			...
			if (this._impl != null) {
				...
				while (i.hasNext()) {
					...
					Parameter parameter = BridgeFactory.eINSTANCE
							.createParameter();
					parameter.setType(this.processor.getBridgeFactory()
							.buildClassifier(p.getEType()));
