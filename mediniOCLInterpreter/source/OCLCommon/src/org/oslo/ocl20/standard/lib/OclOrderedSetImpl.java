...
public class OclOrderedSetImpl extends OclCollectionImpl implements
		OclOrderedSet {
	public OclOrderedSetImpl(Classifier eT, Object[] array,
	...
	public OclType oclType() {
		Classifier type = adapter.getProcessor().getTypeFactory()
				.buildOrderedSetType(super.getElementType());
	...
	public OclBag union(OclBag bag) {
		OclBagImpl b = (OclBagImpl) adapter.Bag(this.getElementType(),
				orderedset_impl());
	...
	public OclOrderedSet subOrderedSet(OclInteger lower, OclInteger upper) {
		if (((OclIntegerImpl) lower).int_impl() < 1)
			return null;
		if (((OclIntegerImpl) upper).int_impl() > ((OclIntegerImpl) this.size())
				.int_impl())
			return null;
		int l = ((OclIntegerImpl) lower).int_impl() - 1;
		int u = ((OclIntegerImpl) upper).int_impl() - 1 + 1;
		return adapter.OrderedSet(this.getElementType(), orderedset_impl()
				.subList(l, u));
	}

	public OclAny at(OclInteger i) {
		return (OclAny) orderedset_impl().get(
				((OclIntegerImpl) i).int_impl() - 1);
	}
		...
		while (i.hasNext()) {
			...
			if (elem instanceof OclCollection) {
				...
				if (elem instanceof OclBag) {
					j = ((Collection) ((OclBag) elem).flatten()
							.getImplementation()).iterator();
				} else if (elem instanceof OclOrderedSet) {
					j = ((Collection) ((OclOrderedSet) elem).flatten()
							.getImplementation()).iterator();
				} else if (elem instanceof OclSet) {
					j = ((Collection) ((OclSet) elem).flatten()
							.getImplementation()).iterator();
				} else if (elem instanceof OclSequence) {
					j = ((Collection) ((OclSequence) elem).flatten()
							.getImplementation()).iterator();
				}
				while (j.hasNext()) {
					OclAny x = (OclAny) j.next();
					if (x != null
							&& ((OclBooleanImpl) flat.excludes(x)) == OclBooleanImpl.TRUE)
	...
	public boolean equals(Object o) {
		return (o instanceof OclSet) ? ((OclBooleanImpl) equalTo((OclOrderedSet) o)) == OclBooleanImpl.TRUE
				: false;
	}
