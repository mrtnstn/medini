...
public class BooleanTypeImpl extends PrimitiveImpl implements BooleanType {
	...
	public void createOperations(TypeFactory tf) {
		...
		getOperations().add(
				bf.buildOperation(tf.buildBooleanType(), "or",
						new Classifier[] { tf.buildBooleanType(), }));
		getOperations().add(
				bf.buildOperation(tf.buildBooleanType(), "xor",
						new Classifier[] { tf.buildBooleanType(), }));
		getOperations().add(
				bf.buildOperation(tf.buildBooleanType(), "and",
						new Classifier[] { tf.buildBooleanType(), }));
		getOperations().add(
				bf.buildOperation(tf.buildBooleanType(), "not",
						new Classifier[] {}));
		getOperations().add(
				bf.buildOperation(tf.buildBooleanType(), "implies",
						new Classifier[] { tf.buildBooleanType(), }));
