...
/**
 * This iterator class is an optimized variant of
 * {@link QvtSemanticBindingIterator}. {@link QvtSemanticBindingIterator} has
 * exponential runtime behavior to the total number of nested object templates
 * of the source domains of a relation. This implementation has a runtime
 * behavior near the possible number of bindings for a trace.
 * 
 * @author kiegeland
 */
...
public class QvtOptimizedSemanticBindingIterator implements Iterator {
	...
	/**
	 * If <code>hasNextBinding</code> is true, there is a binding available at
	 * any case
	 */
	private boolean hasNextBinding;

	/**
	 * If <code>hasNoBinding</code> is true, there is no binding available at
	 * any case
	 */
	 ...
	 *            NOTE: This order is supposed: E.g. if template A is nested in
	 *            template B, index(A)>index(B)
	 * @param processor
	 * @param rootValues
	 *            a mapping of the source domain variables to the trace values
	 */
	public QvtOptimizedSemanticBindingIterator(List templatesToIterate,
			QvtProcessorImpl processor, Map rootValues) {
		this.processor = processor;
		this.objectTemplates = (ObjectTemplateExp[]) templatesToIterate
				.toArray(new ObjectTemplateExp[templatesToIterate.size()]);
		this.allIterators = new Iterator[templatesToIterate.size()];
		this.currentObjects = rootValues;
		this.hasNextBinding = this.initializeIterators(0); // Note, offer one
															// binding if no
															// nested
		// templates exist
	...
	public boolean hasNext() {
		...
		for (int i = this.allIterators.length - 1; i >= 0; i--) {
			while (this.allIterators[i] != null) {
				if (this.allIterators[i].hasNext()) {
					this.currentObjects.put(this.objectTemplates[i],
							this.allIterators[i].next());
					...
					if (this.failedClause == null) {
						this.failedClause = this.objectTemplates[i];
					}
					break;
				}
			}
	...
	public Object next() {
		if (this.hasNext()) {
			...
			for (ObjectTemplateExp objectTemplate : this.objectTemplates) {
				OclAny objectTemplateValue = (OclAny) this.currentObjects
						.get(objectTemplate);
				if (objectTemplateValue == null) {
					throw new RuntimeException(
							"Internal error in QvtOptimizedSemanticBindingIterator: binding var is null");
				}
				runtimeEnvironment.setValue(objectTemplate.getBindsTo()
						.getName(), objectTemplateValue);
			}
	...
	/**
	 * Initialized all iterators of templates "template" where
	 * index(template)>=fromIndex. An iterator is initialized with the values
	 * offered by the left side of the PropertyTemplateItem which has "template"
	 * on its right side.
	 * 
	 * @param fromIndex
	...
	private boolean initializeIterators(int index) {
		...
		if (this.allIterators[index] != null
				&& this.allIterators[index].hasNext()) {
			throw new RuntimeException(
					"Internal error in QvtOptimizedSemanticBindingIterator: child has still bindings");
		}
		if (template.eContainer() instanceof PropertyTemplateItem) {
			PropertyTemplateItem propertyTemplateItem = (PropertyTemplateItem) template
					.eContainer();
			ObjectTemplateExp parentTemplate = propertyTemplateItem
					.getObjContainer();

			Object sourceValue = this.currentObjects.get(parentTemplate);
			List vals = QVTDirectedValidation.getValueList(sourceValue,
			...
			for (Iterator iter = vals.iterator(); iter.hasNext();) {
				...
				OclAny currentValue = this.processor.getStdLibAdapter().OclAny(
						obj);

				if (!(currentValue == null || currentValue instanceof OclUndefined)
						&& currentValue.oclType().asJavaObject() == null) {
					throw new RuntimeException(
							"QvtOptimizedSemanticBindingIterator: cannot retrieve RTTI");
				}
				if (!(currentValue == null || currentValue instanceof OclUndefined)
						&& currentValue.oclType().asJavaObject() != null
						&& !((Classifier) currentValue.oclType().asJavaObject())
								.conformsTo(template.getType()).booleanValue()) {
			...
			while (this.allIterators[index].hasNext()) {
				this.currentObjects.put(template,
						this.allIterators[index].next());
		...
		throw new RuntimeException(
				"Internal error in QvtOptimizedSemanticBindingIterator: Nested object template not part of a PropertyTemplateItem");
	}

	/**
	 * Returns the index of a template in the {@link #objectTemplates} array.
