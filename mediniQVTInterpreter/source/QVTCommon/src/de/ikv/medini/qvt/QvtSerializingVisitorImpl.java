...
public class QvtSerializingVisitorImpl extends OclSerializingVisitorImpl
	...
	public QvtSerializingVisitorImpl(int initialIndentation,
			String indentationSpace, int indentationStep) {
		super(initialIndentation);
		this.indentationSpace = indentationSpace;
		this.indentationStep = indentationStep;
	}

	static public String getVarName(VariableDeclaration varDecl,
			List varsToEmphasis) {
	...
	protected boolean isKeyword(String string) {
		return Arrays.asList(QvtSerializingVisitorImpl.fgQVTkeywords).contains(
				string)
				|| super.isKeyword(string);
	}

	@Override
	public Object visit(VariableExp host, Object data) {
		if (this.varsToEmphasis != null
				&& this.varsToEmphasis.contains(host.getReferredVariable())) {
	...
	public Object visit(Relation host, Object data) {
		...
		vars.removeAll(QVTDirectedValidation.collectAllTemplateVariables(host,
				data, null));
	...
	public Object visit(RelationCallExp host, Object data) {
		Transformation referredTransformation = QVTDirectedValidation
				.getTransformationOf(host.getReferredRelation());
