...
/**
 * This class is based on org.eclipse.ocl.internal.evaluation.QueryImpl,
 * adjusted for the medini OCL engine.
 * 
 * @author kiegeland
 * 
 */
...
public class QueryImpl implements Query {
	...
	/**
	 * If this model list is set, the query can access on the elements in the
	 * model list. If not set, the query can only access on elements which are
	 * in the same resource as the "self" element, or on no elements if there is
	 * no "self" element.
	 */
	 ...
	 * @param declaration
	 * @param extentMap
	 *            may be <code>null</code>, in which case I will use my
	 *            evaluation environment to create a dynamic extent map
	 */
	public QueryImpl(Environment environment, ContextDeclaration declaration,
			Map extentMap, OclProcessorImpl ev) {
	...
	public Object resultType() {
		return ((Constraint) this.expression.getConstraint().get(0))
				.getBodyExpression().getType();
	}
