...
/**
 * Represents an item of a collection value identified by an index in this
 * collection
 */
...
public class QVTStackFrameCollectionItem extends QVTStackFrameAssignable {
	...
	public OclAny evaluate(IDebugAdapter debugAdapter) {
		if (this.sourceValue != null
				&& this.sourceValue.asJavaObject() instanceof Collection) {
			Collection col = (Collection) this.sourceValue.asJavaObject();
			if (this.index < col.size()) {
				return debugAdapter.getQvtProcessor().getStdLibAdapter()
						.OclAny(new ArrayList(col).get(this.index));
			}
		}
		return null;
	}
	...
	public void assign(IDebugAdapter debugAdapter, OclAny value) {
		if (this.getSourceValue() != null
				&& this.getSourceValue().asJavaObject() instanceof Collection) {
